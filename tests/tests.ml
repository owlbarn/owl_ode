...
module IC = struct
  ...
  let to_symplectic_initial { x0; y0 } =
    Owl.Mat.of_arrays [| [| x0 |] |], Owl.Mat.of_arrays [| [| y0 |] |]
end

let oscillator : Owl.Mat.mat -> float -> Owl.Mat.mat =
 fun xs _t ->
  let j = [| [| 0.; 1. |]; [| -1.; 0. |] |] |> Owl.Mat.of_arrays in
  Owl.Mat.(j *@ xs)


let oscillator_symplectic : Owl.Mat.mat * Owl.Mat.mat -> float -> Owl.Mat.mat =
...
let test_native algo ord tspec exact ic : int =
  let t, sol = algo oscillator (IC.to_initial ic) tspec () in
  let open Owl in
...
let test_symplectic algo ord tspec exact ic : int =
  let t, sol, _ = algo oscillator_symplectic (IC.to_symplectic_initial ic) tspec () in
  let open Owl in
...
let native test list () =
  let tester (algo, ord, name) =
    Alcotest.(check @@ int)
      ("sin " ^ name)
      (ord + dt_ord)
      (test algo ord tspec Owl.Maths.sin sin_ic);
    Alcotest.(check @@ int)
      ("cos " ^ name)
      (ord + dt_ord)
      (test algo ord tspec Owl.Maths.cos cos_ic)
  in
  List.iter tester list
...
(* Run it *)
let () = Alcotest.run "owl-ode test set" [ "test_set", test_set ]
