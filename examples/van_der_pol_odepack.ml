...
let () =
  ...
  (* save ts and ys *)
  Mat.save_txt Mat.(ts @|| ys) ~out:"van_der_pol_dynamics_o.txt"
...
let () =
  ...
  (* save ts and ys *)
  Mat.save_txt Mat.(ts @|| ys) ~out:"van_der_pol_dynamics_custom_o.txt";
  ...
  legend_on h ~position:NorthEast [| "LSODA"; "RK4"; "RK45" |];
  subplot h 1 0;
  ...
  legend_on h ~position:NorthEast [| "LSODA"; "LSOAD"; "RK4"; "RK4"; "RK45"; "RK45" |];
  output h
